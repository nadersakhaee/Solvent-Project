...
dict_smp_y, smp_y, tags_smp_y, np_smp_y = create_dict_data_tag('nTrsry/Y_OOS_CHactiv_121.csv')
...
#######################                c_56_n85                ######################
########################################################################################
oscn, tscn, rscn, fscn =    '6',       '7' ,       '5',        '7'
ocn, tcn, rcn, fcn     =      6,        7 ,          5,          7
olim, tlim, rlim, flim =   593775,    2035800,       142506,    2035800
osn, tsn, rsn, fsn     =    144,      497,       34,      497
osna, tsna, rsna, fsna =    18,      62,       4,      62
oinia, tinia, rinia, finia =    0,      540758,       73158,      15904
ofina, tfina, rfina, ffina =    144,      541255,       73192,      16401
GenX = "c_56_n85"
...
h5f = h5py.File('nTrsry/HDFs/np_cmb_p1_'+oscn+'.h5','r')
np_cmb_p1_o = h5f['np_cmb_p1_'+oscn][:]
h5f.close()

h5f = h5py.File('nTrsry/HDFs/np_cmb_p2_'+tscn+'.h5','r')
np_cmb_p2_t = h5f['np_cmb_p2_'+tscn][:]
h5f.close()

h5f = h5py.File('nTrsry/HDFs/np_cmb_p3_'+rscn+'.h5','r')
np_cmb_p3_r = h5f['np_cmb_p3_'+rscn][:]
h5f.close()

h5f = h5py.File('nTrsry/HDFs/np_cmb_p4_'+fscn+'.h5','r')
np_cmb_p4_f = h5f['np_cmb_p4_'+fscn][:]
h5f.close()
...
# h5f = h5py.File('nTrsry/HDFs/np_cmb_p4_4.h5','r')
# np_cmb_p4_4 = h5f['np_cmb_p4_4'][:]
# # print(f"shape of cmb_33_4 uploaded array is{np.shape(np_cmb_p4_4)}\n\n")
# h5f.close()

# with h5py.File('nTrsry/np_cmb_p4_4.h5','r') as h5f:
...
index_combinations = []
# for o in range(0,olim, osn):
#     for t in range(0,tlim, tsn):
#         for r in range(0,rlim, rsn):
#             for f in range(0,flim, fsn):
for o in range(oinia,ofina, osna):
    for t in range(tinia,tfina, tsna):
        for r in range(rinia,rfina, rsna):
            for f in range(finia,ffina, fsna): 
                # print(f"{t},{i},{b},{o}")
                # print(f"{np_cmb_p1_5[t]},{np_cmb_p2_6[i]},{np_cmb_p3_8[b]},{np_cmb_p4_4[o]}")
                index_combinations.append( [[int(o),int(t),int(r),int(f)] ,[np.concatenate((np_cmb_p1_o[o], np_cmb_p2_t[t], np_cmb_p3_r[r], np_cmb_p4_f[f])).astype(int).tolist()]])
                # print(type(np.concatenate((np_cmb_p1_5[t], np_cmb_p2_6[i], np_cmb_p3_8[b], np_cmb_p4_4[o])).astype(int).tolist()))
...
picked_lis_y_pred_train, picked_lis_y_pred_test, picked_lis_y_train, picked_lis_y_test, lis_trn_id, lis_tst_id, otrf_cods, otrf_itrs, pwcs = [], [], [], [], [], [], [], [], []
    ...
    ##############################################################################################
        for w2, m in enumerate(models):
            ...
            if m.name not in ["linRdige","Huber"]:
                ...
                if mae_t < 9 and r2_t > 0.745 and  intercept < 60:        # @@@@@@@@@@@@@@@@@@@@@    !!!!!!!     SWITCH HERE        !!!!!!!!!     @@@@@@@@@@@@@@@@@
                ...
                # if mae_t < 12 and r2_t > 0.56 and q2 > 0.37:         # @@@@@@@@@@@    !!!!!!!     SWITCH HERE        !!!!!!!!!     @@@@@@@@@
                    # otrf_itrs.append("_".join([str(int(ind_lis[0][0] // osna)),str(int(ind_lis[0][1] // tsna)),str(int(ind_lis[0][2] // rsna)),str(int(ind_lis[0][3] // fsna))]))
                    otrf_itrs.append("_".join(str(y) for y in [int(ind_lis[0][0] // osna), int(ind_lis[0][1] // tsna),int(ind_lis[0][2] // rsna), int(ind_lis[0][3] // fsna) ]))
                    otrf_cods.append("_".join(str(x) for x in ind_lis[0][:]))
                    pwcs.append(pw1)
                    ...
                    plt.xlabel("Observed yields for CH_activation (Y_Obsv's Data)  ")
                    plt.ylabel("Predicted yields for CH_activation (Y_Pred's Data) ")
                    # plt.ylim(0, 100)
                    # plt.xlim(0, 100)                    
                    plt.ylim(-15, 100)
                    plt.xlim(-15, 100)
...
with open(f"Plots/{GenX}/depo/TrnCombTrsry.csv", mode="w", newline="") as file:
    ...
    for j , trainIDs in enumerate(lis_trn_id):
        writer.writerow([otrf_cods[j]])
        writer.writerow([otrf_itrs[j]])
